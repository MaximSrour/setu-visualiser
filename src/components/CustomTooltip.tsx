	...
	if (active && payload?.length) {
		...
		return (
			<div className="bg-slate-800 rounded-md p-3">
				...
				{false && <p>{data.year} - {data.semester}</p>}
				{false && <p>{data.aspectDefinition.description}</p>}
...
export function CustomizedDot(props: any) {
	...
	if (value >= 3.8) {
	return (
		<svg x={props.cx ?? 0 - 10} y={props.cy ?? 0 - 10} width={20} height={20} fill="red" viewBox="0 0 1024 1024">
	...
	return (
	<svg x={props.cx ?? 0 - 10} y={props.cy ?? 0 - 10} width={20} height={20} fill="green" viewBox="0 0 1024 1024">
