...
export default function Traffic(props: IOfferingsAspectRadarChartProps) {
	if(!props.dataCurr) { return null; }
	if(!props.dataPrev) { return null; }
	...
	const dataDiff = props.dataCurr.map((curr, index) => {
	...
	}).filter((d) => d !== null) as unknown as { aspect: number, median: number }[];
