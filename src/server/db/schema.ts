...
export const offerings = mysqlTable(
	"offerings",
	{
		...
		campus: varchar("campus", { length: 2 }).notNull(),
...
export const aspectData = mysqlTable(
	"aspect_data",
	{
		...
		campus: varchar("campus", { length: 2 }).notNull(),
		mode: varchar("mode", { length: 32 }).notNull(),
		aspectType: varchar("aspect_type", { length: 2 }).notNull(),
		aspect: smallint("aspect").notNull(),
...
export const aspectDefinitions = mysqlTable(
	...
	(table) => {
		return {
			pk: primaryKey({
				name: "id",
				columns: [table.aspectType, table.aspect],
			}),
		};
	},
);

export const aspectDefinitionsRelations = relations(
	aspectDefinitions,
	({ many }) => ({
		aspectData: many(aspectData),
	}),
